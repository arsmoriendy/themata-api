...
impl DB {
    pub async fn read_theme(&self, ulid: &Ulid) -> Result<Option<ReadData>, SqlxError> {
        query_as(
            "SELECT name, schemes, owner, description, count(likes) AS like_count, views, \
             created_at FROM themes LEFT JOIN likes ON themes.ulid = likes.theme_ulid GROUP BY \
             themes.ulid HAVING ulid = $1",
        )
        .bind(ulid)
    ...
    pub async fn list_themes<'a>(
        ...
        let mut q = QueryBuilder::<Postgres>::new(
            "SELECT ulid, name, schemes, owner, description, count(likes) AS like_count, views, \
             created_at FROM themes LEFT JOIN likes ON themes.ulid = likes.theme_ulid",
        );
